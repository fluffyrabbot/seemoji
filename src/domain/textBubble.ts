...
export interface TextBubble { readonly kind: BubbleKind; readonly speakerId?: string; readonly speakerAnchor?: { readonly x: number; readonly y: number }; readonly tail: { readonly x: number; readonly y: number } }
...
export function setTextBubble(layer: TextLayer, kind: BubbleKind | 'plain'): TextLayer {
  ...
  return { ...layer, bubble: { kind, ...(previous?.speakerId ? { speakerId: previous.speakerId, ...(previous.speakerAnchor ? { speakerAnchor: previous.speakerAnchor } : {}) } : {}), tail: previous?.tail ?? {
